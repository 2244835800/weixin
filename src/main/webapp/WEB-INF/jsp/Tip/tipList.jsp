...
<table width="100%" border="0" bgcolor="#ffffff" cellpadding="0" cellspacing="0">
 ...
 <c:if test="${id==1}">政务公开</c:if>
    ...
    <c:if test="${id==6}">信息咨询</c:if>
    <c:if test="${id==7}">网上客服</c:if>
    <c:if test="${id==8}">留言</c:if>   
   </div>
    <div class="zx_list">
    <ul>
   ...
   <span>
    ...
    <li ><a href="<%=path%>/TipMsg/detail?tipId=${list.strTipId}" class="li">${list.strTipDescription}</a>
    	<span>${list.commitTime}</span></li>
...
<c:if test="${page.hasFirst==true}"><span class="page_down"><a href="<%=path%>/TipMsg/tipList?pageNow=1">首页</a></span></c:if>
<c:if test="${page.hasFirst==false}"><span >首页</span></c:if>
<c:if test="${page.hasPre==true}"><span class="page_down"><a href="<%=path%>/TipMsg/tipList?pageNow=${page.pageNow-1}">上一页</a></span></c:if>
<c:if test="${page.hasPre==false}"><span >上一页</span></c:if>
<c:if test="${page.hasPre==true}"><span class="page_nom"><a href="<%=path%>/TipMsg/tipList?pageNow=${page.pageNow-1}">${page.pageNow-1}</a></span></c:if>
<span class="page_cur">${page.pageNow}</span>
<c:if test="${page.hasNext==true}"><span class="page_nom"><a href="<%=path%>/TipMsg/tipList?pageNow=${page.pageNow+1}">${page.pageNow+1}</a></span></c:if>
<c:if test="${page.hasNext==true}"><span class="page_down"><a href="<%=path%>/TipMsg/tipList?pageNow=${page.pageNow+1}">下一页</a></span></c:if>
<c:if test="${page.hasNext==false}"><span >下一页</span></c:if>
<c:if test="${page.hasLast==true}"><span class="page_down"><a href="<%=path%>/TipMsg/tipList?pageNow=${page.totalPageCount}">尾页</a></span></c:if>
