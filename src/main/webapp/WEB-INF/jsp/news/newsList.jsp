  ...
  <li class=""><a href="<%=path%>/index/menu?id=3&type=1" target="_self">中心动态</a>
   ...
   <li><a href="<%=path%>/TipMsg/TipMsg" target="_self">举报投诉</a>
               ...
               <ul><li><a href="<%=path%>/TipMsg/TipMsg" target="_self">违建举报</a> </li>
                   <li><a href="<%=path%>/news/newsList?pageNow=1" target="_self">举报信息</a> </li>
               </ul>
               </div>
              <div class="xlbot"></div></div>
   </li>
...
<table width="100%" border="0" bgcolor="#ffffff" cellpadding="0" cellspacing="0">
 ...
 <c:if test="${id==1}">政务公开</c:if>
    ...
    <c:if test="${id==5}"><li><a href="<%=path%>/TipMsg/TipMsg">违建举报</a></li><li><a href="<%=path%>/news/newsList?pageNow=1">举报信息</a></li></c:if>
    <c:if test="${id==6}"></c:if>
    </ul>
    </div>
   ...
   <c:forEach var="list" items="${list}" varStatus="status">
    <li ><a href="<%=path%>/news/newsDetail?newsId=${list.newsId}" class="li">${list.newsTitle}</a>
...
<div class="manu">
<c:if test="${page.hasFirst==true}"><span class="page_down"><a href="<%=path%>/news/newsList?pageNow=1">首页</a></span></c:if>
<c:if test="${page.hasFirst==false}"><span >首页</span></c:if>
<c:if test="${page.hasPre==true}"><span class="page_down"><a href="<%=path%>/news/newsList?pageNow=${page.pageNow-1}">上一页</a></span></c:if>
<c:if test="${page.hasPre==false}"><span >上一页</span></c:if>
<c:if test="${page.hasPre==true}"><span class="page_nom"><a href="<%=path%>/news/newsList?pageNow=${page.pageNow-1}">${page.pageNow-1}</a></span></c:if>
<span class="page_cur">${page.pageNow}</span>
<c:if test="${page.hasNext==true}"><span class="page_nom"><a href="<%=path%>/news/newsList?pageNow=${page.pageNow+1}">${page.pageNow+1}</a></span></c:if>
<c:if test="${page.hasNext==true}"><span class="page_down"><a href="<%=path%>/news/newsList?pageNow=${page.pageNow+1}">下一页</a></span></c:if>
<c:if test="${page.hasNext==false}"><span >下一页</span></c:if>
<c:if test="${page.hasLast==true}"><span class="page_down"><a href="<%=path%>/news/newsList?pageNow=${page.totalPageCount}">尾页</a></span></c:if>
